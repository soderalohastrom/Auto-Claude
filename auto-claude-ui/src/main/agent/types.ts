...
export interface SpecCreationMetadata {
  ...
  //
  // NOTE: We allow Z.ai GLM model identifiers here so task creation metadata can carry
  // the selected model string when routing via ANTHROPIC_BASE_URL / ANTHROPIC_AUTH_TOKEN.
  // Phase models remain Claude-only ('haiku'|'sonnet'|'opus') because the Auto profile
  // is tightly coupled to Claude's per-phase shorthand model system.
  model?: "haiku" | "sonnet" | "opus" | "glm-4.7" | "glm-4.5-air";
